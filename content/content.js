  ...
  for (const selector of selectors) {
    ...
    for (const node of nodes) {
      ...
      for (const value of values) {
        const language = findExactLanguageInText(String(value));
        if (language) {
          return language;
        }
      }
...
function extractLeetCodeCode() {
  // DOM-based fallback — the bridge script handles the primary capture
  // via the Monaco editor API (see injectEditorBridge below).

  // 1. Try textarea (some LeetCode layouts)
  ...
  // 3. Collect visible .view-line elements (partial, but last resort)
...
function extractLeetCodeLanguage() {
  const codeLanguage = inferLanguageFromCode(extractLeetCodeCode());
  if (codeLanguage) {
    return codeLanguage;
  }
...
function extractGfgCode() {
  ...
  // Last resort: ace_content textContent (will lose newlines, but at least captures code)
...
function extractGfgLanguage() {
  // 1. Infer language from code content using regex heuristics (most reliable)
  const codeLanguage = inferLanguageFromCode(extractGfgCode());
  if (codeLanguage) {
    return codeLanguage;
  }

  // 2. Try UI selectors (language dropdown, labels, etc.)
  const gfgLangSelectors = [
    "select",
  ...
  for (const selector of gfgLangSelectors) {
    const nodes = Array.from(document.querySelectorAll(selector));
    for (const node of nodes) {
      const values = [
        node.value,
      ...
      for (const value of values) {
        const language = findExactLanguageInText(String(value).trim());
        if (language) {
          return language;
        }
      }
  ...
  // 3. Fallback: try matching known language names in body text
  const bodyText = document.body?.innerText || "";
...
function detectLeetCodeSolution() {
  ...
  return {
    ...
    language: extractLeetCodeLanguage(),
...
function detectGfgSolution() {
  return {
    ...
    code: extractGfgCode(),
    language: extractGfgLanguage(),
    tags: extractTags(".tag-card, .problemTopic, a[href*='/tag/'], a[href*='/tag-list/']")
  };
}

// Inject editor-bridge.js into the page's MAIN world to access Monaco/Ace APIs.
...
async function captureCurrentSolution() {
  ...
  if (platform !== PLATFORMS.LEETCODE && platform !== PLATFORMS.GFG) {
    return null;
  }

  // First, try to get full code from the editor API via the bridge script
  ...
  // If the bridge got full code, prefer it over the DOM-based extraction
  if (bridgeCode?.trim()) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === "CAPTURE_CURRENT_SOLUTION") {
    captureCurrentSolution().then((solution) => {
      const validSolution = solution?.title && solution?.code ? solution : null;
      sendResponse({ ok: Boolean(validSolution), solution: validSolution });
    });
